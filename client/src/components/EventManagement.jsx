  ...
  const handleFetchRounds = async () => {
    try {
      ...
      const processedRounds = roundsData.map((round) => {
        console.log("Processing round:", round);
        const heats = round.heats && Array.isArray(round.heats) ? round.heats : [];
  
        const processedHeats = heats.map((heat) => {
          console.log("Processing heat:", heat);
          const competitors = heat.competitors && Array.isArray(heat.competitors)
