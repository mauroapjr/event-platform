...
router.post("/save-rounds", async (req, res) => {
  ...
  try {
    ...
    if (existingRoundIds.length > 0) {
      ...
      await client.query("DELETE FROM heats WHERE round_id = ANY($1::int[])", [
        existingRoundIds,
      ]);
      await client.query("DELETE FROM rounds WHERE id = ANY($1::int[])", [
        existingRoundIds,
      ]);
    }
...
router.post("/generate-rounds-pdf", async (req, res) => {
  ...
  try {
    // Fetch rounds data directly from the database to ensure it's up to date
    const roundsResult = await pool.query(
      "SELECT * FROM rounds WHERE event_id = $1",
      [eventId]
    );
    ...
    for (const round of rounds) {
      doc.fontSize(16).text(`Round: ${round.round_name}`, { align: "left" });
      ...
      for (const heat of heats) {
        doc.fontSize(14).text(`Heat: ${heat.heat_name}`, { align: "left" });
        doc.fontSize(12).text(`Competitors:`, { align: "left" });
        const competitors = heat.competitors || [];
